...
public class BasicWeightInputDialog extends OkCancelOptionDialog implements ActionListener {
	...
	private void doInsertDot() {
		//if (Optional.ofNullable(isFloatingPoint() && tfNumber.getText().indexOf('.') < 0) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if (Optional.ofNullable(actionCommand.equals(POSConstants.CLEAR_ALL)) {
			doClearAll();
		}
	...
	private boolean validate(String str) {
		if (Optional.ofNullable(isFloatingPoint()) {
			try {
	...
	public void setValue(double value) {
		...
		else if (Optional.ofNullable(isFloatingPoint()) {
