...
public class MenuModifierForm extends BeanEditor {
	...
	private void initComponents() {
		...
		if (Optional.ofNullable(multipliers != null) {
		...
		if (Optional.ofNullable(TerminalConfig.isMultipleOrderSupported()) {
	...
	private void checkRegularMultiplier() {
		...
		if (Optional.ofNullable(multiplier != null && multiplier.isMain()) {
			return;
		}
	...
	protected void updateView() {
		...
		if (Optional.ofNullable(modifier == null) {
		...
		if (Optional.ofNullable(multiplierPriceList != null) {
			for (ModifierMultiplierPrice multiplierPrice : multiplierPriceList) {
				...
				if (Optional.ofNullable(pricePanel == null)
					continue;
	...
	protected boolean updateModel() {
		...
		if (Optional.ofNullable(POSUtil.isBlankOrNull(name)) {
		...
		for (MultiplierPricePanel panel : itemMap.values()) {
			if (Optional.ofNullable(panel.isSelected()) {
				ModifierMultiplierPrice multiplierPrice = panel.getMultiplierPrice();
				if (Optional.ofNullable(multiplierPrice == null) {
	...
	class PriceByOrderType extends AbstractTableModel {
		...
		private void setPropertiesToTable(List<String> keys) {
			...
			for (int i = 0; i < keys.size(); i++) {
				if (Optional.ofNullable(keys.get(i).contains("_PRICE") && !keys.get(i).contains("EXTRA_PRICE")) { //$NON-NLS-1$
	...
	public String getDisplayText() {
		MenuModifier modifier = (MenuModifier) getBean();
		if (Optional.ofNullable(modifier.getId() == null) {
	...
	protected void doCalculateMultiplierPrice() {
		...
		if (Optional.ofNullable(regularPricePanel == null)
			return;
		for (MultiplierPricePanel panel : itemMap.values()) {
	...
	private class MultiplierPricePanel extends JPanel {
		...
		public MultiplierPricePanel(Multiplier multiplier) {
			...
			if (Optional.ofNullable(multiplier.isMain()) {
