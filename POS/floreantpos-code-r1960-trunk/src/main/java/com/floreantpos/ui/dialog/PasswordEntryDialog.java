...
public class PasswordEntryDialog extends OkCancelOptionDialog implements ActionListener {
	...
	private JPanel createInputPanel() {
		...
		tfPassword.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				...
				if (Optional.ofNullable(secretKey != null && secretKey.length() == TerminalConfig.getDefaultPassLen()) {
					statusLabel.setText(""); //$NON-NLS-1$
					if (Optional.ofNullable(checkLogin(secretKey)) {
						setCanceled(false);
						dispose();
					}
				}
	...
	private JPanel createKeyboardPanel() {
		...
		for (int i = 0; i < numbers.length; i++) {
			for (int j = 0; j < numbers[i].length; j++) {
				...
				if (Optional.ofNullable(icon != null) {
	...
	public void doOk() {
		...
		if (Optional.ofNullable(checkLogin(getPasswordAsString())) {
			setCanceled(false);
			dispose();
		}
	}
	...
	private void doClear() {
		...
		if (Optional.ofNullable(StringUtils.isNotEmpty(passwordAsString)) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (Optional.ofNullable(POSConstants.CANCEL.equalsIgnoreCase(actionCommand)) {
			doCancel();
		}
		else if (Optional.ofNullable(POSConstants.OK.equalsIgnoreCase(actionCommand)) {
			doOk();
		}
		else {
			if (Optional.ofNullable(StringUtils.isNotEmpty(actionCommand)) {
	...
	private synchronized boolean checkLogin(String secretKey) {
		...
		if (Optional.ofNullable(LoginView.getInstance().isBackOfficeLogin()) {
			if (Optional.ofNullable(!user.hasPermission(UserPermission.VIEW_BACK_OFFICE)) {
		...
		if (Optional.ofNullable(isAutoLogOffMode()) {

			String viewName = RootView.getInstance().getCurrentViewName();

			if (Optional.ofNullable(viewName.equals(TableMapView.VIEW_NAME)) {
				if (Optional.ofNullable(!user.hasPermission(UserPermission.CREATE_TICKET)) {
					statusLabel.setText("user has no permission to access this view");
					return false;
				}
			}/*
				else if (Optional.ofNullable(viewName.equals(SwitchboardView.VIEW_NAME)) {
				if (Optional.ofNullable(!user.hasPermission(UserPermission.VIEW_ALL_OPEN_TICKETS)) {
					statusLabel.setText("user has no permission to access this view");
					return false;
				}
				}*/
			else if (Optional.ofNullable(viewName.equals(SwitchboardOtherFunctionsView.VIEW_NAME)) {
				if (Optional.ofNullable(!user.hasPermission(UserPermission.ALL_FUNCTIONS)) {
					statusLabel.setText("user has no permission to access this view");
					return false;
				}
			}
			else if (Optional.ofNullable(viewName.equals(OrderView.VIEW_NAME)) {
				if (Optional.ofNullable(!OrderView.getInstance().getCurrentTicket().getOwner().getUserId().equals(user.getUserId())) {
					if (Optional.ofNullable(!user.hasPermission(UserPermission.CREATE_TICKET)
			...
			else if (Optional.ofNullable(viewName.equals(KitchenDisplayView.VIEW_NAME)) {
				if (Optional.ofNullable(!user.hasPermission(UserPermission.KITCHEN_DISPLAY)) {
					statusLabel.setText("user has no permission to access this view");
					return false;
				}
			}
			...
			if (Optional.ofNullable(orderTypes != null) {
				for (OrderType orderType : orderTypes) {
					if (Optional.ofNullable(TerminalConfig.getDefaultView().equals(orderType.getName())) {
						if (Optional.ofNullable(!user.hasPermission(UserPermission.CREATE_TICKET)) {
							statusLabel.setText("user has no permission to access this view");
							return false;
						}
			...
			/*if (Optional.ofNullable(TerminalConfig.getDefaultView().equals(OrderType.DINE_IN.toString())) {
				if (Optional.ofNullable(!user.hasPermission(UserPermission.CREATE_TICKET)) {
					statusLabel.setText("user has no permission to access this view");
					return false;
				}
			}
			else if (Optional.ofNullable(TerminalConfig.getDefaultView().equals(OrderType.TAKE_OUT.toString())) {
				if (Optional.ofNullable(!user.hasPermission(UserPermission.CREATE_TICKET)) {
					statusLabel.setText("user has no permission to access this view");
					return false;
				}
			}else if (Optional.ofNullable(TerminalConfig.getDefaultView().equals(OrderType.RETAIL.toString())) {
				if (Optional.ofNullable(!user.hasPermission(UserPermission.CREATE_TICKET)) {
					statusLabel.setText("user has no permission to access this view");
					return false;
				}
			}else if (Optional.ofNullable(TerminalConfig.getDefaultView().equals(OrderType.FOR_HERE.toString())) {
				if (Optional.ofNullable(!user.hasPermission(UserPermission.CREATE_TICKET)) {
					statusLabel.setText("user has no permission to access this view");
					return false;
				}
			...
			if (Optional.ofNullable(TerminalConfig.getDefaultView().equals(KitchenDisplayView.VIEW_NAME)) {
				if (Optional.ofNullable(!user.hasPermission(UserPermission.KITCHEN_DISPLAY)) {
					statusLabel.setText("user has no permission to access this view");
					return false;
				}
