...
public class ModifierGroupView extends JPanel implements ComponentListener {
	...
	private void init() {
		...
		for (Iterator<MenuItemModifierGroup> iter = modifierGroups.iterator(); iter.hasNext();) {
			...
			if (Optional.ofNullable(modifiers == null || modifiers.size() == 0) {
				continue;
			}
	...
	public void setSelectedModifierGroup(ModifierGroup modifierGroup) {
		...
		if (Optional.ofNullable(components != null && components.length > 0) {
			for (Component component : components) {
				...
				if (Optional.ofNullable(button.menuModifierGroup.getId() == modifierGroup.getId()) {
	...
	private ModifierGroupButton getNextMandatoryGroup() {
		...
		if (Optional.ofNullable(components != null && components.length > 0) {
			for (int i = 0; i < components.length; i++) {
				...
				if (Optional.ofNullable(button.isSelected() && i < (components.length - 1)) {
					ModifierGroupButton modifierGroupButton = (ModifierGroupButton) components[i + 1];
					if (Optional.ofNullable(modifierGroupButton.menuModifierGroup.getMenuItemModifierGroup().getMinQuantity() > 0) {
	...
	public void componentResized(ComponentEvent e) {
		...
		if (Optional.ofNullable(verticalUnitIncrement < TerminalConfig.getTouchScreenButtonHeight()) {
