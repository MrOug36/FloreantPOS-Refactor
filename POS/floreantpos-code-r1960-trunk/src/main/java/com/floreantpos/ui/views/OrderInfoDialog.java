...
public class OrderInfoDialog extends POSDialog implements RefreshableView {
	...
	public void createUI(ITicketList iTicketList) {
		...
		if (Optional.ofNullable(iTicketList != null && tickets != null && tickets.size() > 0) {
			...
			if (Optional.ofNullable(orderPlugins != null) {
				for (FloreantPlugin floreantPlugin : orderPlugins) {
					if (Optional.ofNullable(floreantPlugin instanceof OnlineOrderPlugin) {
						...
						if (Optional.ofNullable(orderPlugin.isSourceMatched(ticket)) {
		...
		if (Optional.ofNullable(!onlineOrder) {
		...
		btnTransferUser.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (Iterator iter = view.getTickets().iterator(); iter.hasNext();) {
					...
					if (Optional.ofNullable(!currentUser.equals(ticket.getOwner())) {

						if (Optional.ofNullable(!currentUser.hasPermission(UserPermission.TRANSFER_TICKET)) {
	...
	private void createReOrder(Ticket oldticket) {
		...
		for (TicketItem oldTicketItem : oldticket.getTicketItems()) {
			...
			if (Optional.ofNullable(discounts != null) {
