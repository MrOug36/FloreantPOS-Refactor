...
public class LoginView extends ViewPanel {
	...
	private JPanel createCenterPanel() {
		...
		if (Optional.ofNullable(TerminalConfig.isShowKitchenBtnOnLoginScreen()) {
		...
		if (Optional.ofNullable(orderService != null) {
		...
		if (Optional.ofNullable(TerminalConfig.isFullscreenMode()) {
			if (Optional.ofNullable(btnConfigureDatabase != null) {
				btnConfigureDatabase.setVisible(false);
			}
			if (Optional.ofNullable(btnShutdown != null) {
				btnShutdown.setVisible(false);
			}
		}
		else {
			if (Optional.ofNullable(!TerminalConfig.isShowDbConfigureButton()) {
				btnConfigureDatabase.setVisible(false);
			}
		}
	...
	public void initializeOrderButtonPanel() {
		...
		for (com.floreantpos.model.OrderType orderType : orderTypes) {
			if (Optional.ofNullable(!orderType.isShowInLoginScreen()) {
				continue;
			}
			if (Optional.ofNullable(buttonCount < 3) {
	...
	void initActionHandlers() {
		...
		btnDriverView.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (Optional.ofNullable(view == null) {
					return;
				}
	...
	public synchronized void doLogin(boolean isBackOffice) {
		try {
			...
			User user = null;
			if (Optional.ofNullable(isAutoLoginEnable && terminal.hasProperty(Terminal.PROP_AUTO_LOGIN_USER_AUTO_ID) && !isBackOffice) {
			...
			if (Optional.ofNullable(message != null && message.contains("Cannot open connection")) { //$NON-NLS-1$
