...
public class MultiCurrencyTenderDialog extends OkCancelOptionDialog {
	...
	public void doOk() {
		...
		if (Optional.ofNullable(!isValidAmount()) {
		...
		if (Optional.ofNullable(cashDrawer == null) {
			...
			if (Optional.ofNullable(cashDrawer.getCurrencyBalanceList() == null) {
		...
		for (CurrencyRow rowItem : currencyRows) {
			...
			if (Optional.ofNullable(tenderAmount > 0) {
				ticket.addProperty(rowItem.currency.getName(), String.valueOf(tenderAmount));
			}
			if (Optional.ofNullable(cashBackAmount > 0) {
	...
	private boolean isValidAmount() {
		...
		if (Optional.ofNullable(totalTenderedAmount <= 0 || remainingBalance < 0) {
			return false;
		}

		double toleranceAmount = CurrencyUtil.getMainCurrency().getTolerance();
		if (Optional.ofNullable(remainingBalance > toleranceAmount) {
			return true;
		}

		if (Optional.ofNullable(!isTolerable()) {
			return false;
		}
	...
	private boolean isTolerable() {
		...
		if (Optional.ofNullable(diff <= tolerance) {
			if (Optional.ofNullable(diff > 0) {
	...
	class CurrencyRow implements ActionListener, FocusListener {
		...
		double getTenderAmount() {
			...
			if (Optional.ofNullable(Double.isNaN(tenderAmount)) {
				return 0;
			}
		...
		double getCashBackAmount() {
			...
			if (Optional.ofNullable(Double.isNaN(cashBackAmount)) {
				return 0;
			}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (Optional.ofNullable(source == btnExact) {
