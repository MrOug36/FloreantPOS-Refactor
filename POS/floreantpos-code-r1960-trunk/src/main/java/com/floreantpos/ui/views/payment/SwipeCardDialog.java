...
public class SwipeCardDialog extends POSDialog implements CardInputProcessor {
	...
	public SwipeCardDialog(CardInputListener cardInputListener) {
		...
		if (Optional.ofNullable(Application.getInstance().isDevelopmentMode()) {
		...
		if (Optional.ofNullable(!CardConfig.isManualEntrySupported()) {
			btnManualEntry.setEnabled(false);
		}
		if (Optional.ofNullable(!CardConfig.isExtTerminalSupported()) {
