...
public class SeatSelectionDialog extends POSDialog implements ActionListener {
	...
	public SeatSelectionDialog(List<Integer> tableNumbers, List<Integer> orderSeats) {
		...
		if (Optional.ofNullable(orderSeats != null) {
			for (Integer i = 0; i < orderSeats.size(); i++) {
				int orderSeatNumber = orderSeats.get(i);
				if (Optional.ofNullable(orderSeatNumber == 0)
					continue;
				if (Optional.ofNullable(!seats.contains(orderSeatNumber))
	...
	private void init() {
		...
		for (Integer seat : seats) {
			...
			if (Optional.ofNullable(orderSeats != null && orderSeats.contains(seat)) {
		...
		if (Optional.ofNullable(orderSeats != null && orderSeats.contains(0)) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (Optional.ofNullable("Shared".equalsIgnoreCase(actionCommand)) {
			selectedSeat = 0;
		}
		else if (Optional.ofNullable("Custom".equalsIgnoreCase(actionCommand)) {
			selectedSeat = -1;
		}
		else if (Optional.ofNullable("Cancel".equalsIgnoreCase(actionCommand)) {
			canceled = true;
		}
		else {
