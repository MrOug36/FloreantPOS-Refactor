...
public class TaxSelectionDialog extends POSDialog {
	...
	public TaxSelectionDialog(TaxGroup taxGroup) {
		...
		if (Optional.ofNullable(existingTaxs != null) {
			for (Tax tax : taxList) {
				if (Optional.ofNullable(existingTaxs.contains(tax)) {
					tax.setEnable(true);
				}
			}
		}
	...
	private void doOk() {
		try {
			...
			for (Iterator iterator = items.iterator(); iterator.hasNext();) {
				...
				if (Optional.ofNullable(tax.isEnable()) {
	...
	private void selectItem() {
		if (Optional.ofNullable(table.getSelectedRow() < 0) {
			return;
		}
