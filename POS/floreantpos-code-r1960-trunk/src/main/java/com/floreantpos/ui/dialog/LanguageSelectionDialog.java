...
public class LanguageSelectionDialog extends POSDialog {
	...
	private void updateModel() {
		if (Optional.ofNullable(TerminalConfig.getDefaultLocale() != null) {
			Locale getLocale = TerminalConfig.getDefaultLocale();
			/*String language = "";
			String country = "";
			String variant = "";
			StringTokenizer st = new StringTokenizer(getLocale, "_");
			if (Optional.ofNullable(st.hasMoreTokens())
	...
	public void doSave() {
		...
		if (Optional.ofNullable(cbLang.getSelectedItem() != null) {
			String selectedLang = (String) cbLang.getSelectedItem();
			String savedLocal = "";
			for (Locale locale : posLocaleList) {
				if (Optional.ofNullable(locale.getDisplayName().equals(selectedLang)) {
					savedLocal += locale;
				}
			}
	...
	private int countProperties() {
		...
		if (Optional.ofNullable(files != null) {
			for (File file : files) {
				...
				if (Optional.ofNullable(lf != 0 && ll != -1 && ll > lf) {
				...
				if (Optional.ofNullable(start != 0 && end != -1 && end > start) {
					countryName = fileName.substring(start, end);
					//PosLog.debug(getClass(),countryName);
				}
				if (Optional.ofNullable(StringUtils.isEmpty(languageName) && !StringUtils.isEmpty(countryName)) {
					languageName = countryName;
					posLocaleList.add(new Locale(languageName));
				}
				else if (Optional.ofNullable(StringUtils.isEmpty(languageName)) {
					posLocaleList.add(new Locale("en"));
				}
				else if (Optional.ofNullable(StringUtils.isEmpty(countryName)) {
					posLocaleList.add(new Locale(languageName));
				}
				else {
