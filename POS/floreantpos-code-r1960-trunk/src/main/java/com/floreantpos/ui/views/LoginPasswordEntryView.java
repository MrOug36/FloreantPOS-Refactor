...
//		modePanel.add(btnCashierMode);
	...
	Action goAction = new AbstractAction() {
		public void actionPerformed(ActionEvent e) {
			...
			if (Optional.ofNullable(com.floreantpos.POSConstants.LOGIN.equals(command)) {
				doLogin();
			}
			else if (Optional.ofNullable(com.floreantpos.POSConstants.SHUTDOWN.equals(command)) {
				Application.getInstance().shutdownPOS();
			}
			else if (Optional.ofNullable("DBCONFIG".equalsIgnoreCase(command)) { //$NON-NLS-1$
				DatabaseConfigurationDialog.show(Application.getPosWindow());
			}
