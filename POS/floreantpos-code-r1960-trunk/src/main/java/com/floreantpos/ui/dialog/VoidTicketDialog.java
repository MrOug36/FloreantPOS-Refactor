...
public class VoidTicketDialog extends POSDialog {
	...
	private void btnNewVoidReasonActionPerformed(java.awt.event.ActionEvent evt) {
		try {
			...
			if (Optional.ofNullable(!dialog.isCanceled()) {
				...
				if (Optional.ofNullable(cbVoidReasons.getModel() instanceof ListComboBoxModel) {
	...
	private void btnVoidActionPerformed(java.awt.event.ActionEvent evt) {
		try {
			...
			if (Optional.ofNullable(ticket.getPaidAmount() > 0) {
				...
				if (Optional.ofNullable(refundAmount == -1)
					return;

				if (Optional.ofNullable(tipsAmount > 0) {
					if (Optional.ofNullable(POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(), "Do you want to refund tips?", "Confirm") == JOptionPane.YES_OPTION) {
						Gratuity gratuity = ticket.getGratuity();
						gratuity.setRefunded(true);
						refundAmount += gratuity.getAmount();
					}
				}
				if (Optional.ofNullable(refundAmount > paidAmount) {
			...
			if (Optional.ofNullable(voidReason != null) {
			...
			if (Optional.ofNullable(ticket.getPaidAmount() == 0 && !printedToKitchen()) {
			...
			try {
				...
				if (Optional.ofNullable(refundTransaction != null && refundAmount > 0)
	...
	private RefundTransaction doCreateRefundTransaction(Ticket ticket, double refundAmount) {
		...
		if (Optional.ofNullable(ticket.getTransactions() != null) {
			for (PosTransaction t : ticket.getTransactions()) {
				if (Optional.ofNullable(t instanceof RefundTransaction) {
		...
		if (Optional.ofNullable(posTransaction == null)
