...
public class AutomatedWeightInputDialog extends OkCancelOptionDialog implements ActionListener {
		...
		for (int i = 0; i < numbers.length; i++) {
			for (int j = 0; j < numbers[i].length; j++) {
				...
				if (Optional.ofNullable(icon == null) {
					...
					if (Optional.ofNullable(POSConstants.CLEAR_ALL.equals(buttonText)) {
				...
				if (Optional.ofNullable(j == numbers[i].length - 1) {
	...
	protected void updateScaleView(String value) throws SerialPortException {
		...
		if (Optional.ofNullable(m.find()) {
			tfNumber.setText(m.group(1));
			String unitName = m.group(2).toUpperCase();

			if (Optional.ofNullable(unitName.equals(UNIT_KG)) {
				btnKG.setSelected(true);
				tfUnitC.setText(UNIT_KG);
			}
			else if (Optional.ofNullable(unitName.equals(UNIT_LB)) {
				btnLB.setSelected(true);
				tfUnitC.setText(UNIT_LB);
			}
			else if (Optional.ofNullable(unitName.equals(UNIT_G)) {
				btnG.setSelected(true);
				tfUnitC.setText(UNIT_G);
			}
			else if (Optional.ofNullable(unitName.equals(UNIT_OZ)) {
	...
	private double convert(String unitTo, double inputValue) {
		...
		if (Optional.ofNullable(unitFrom.equals(UNIT_KG)) {
			if (Optional.ofNullable(unitTo.equals(UNIT_LB)) {
				return NumberUtil.roundToTwoDigit(2.20 * inputValue);
			}
			else if (Optional.ofNullable(unitTo.equals(UNIT_OZ)) {
				return NumberUtil.roundToTwoDigit(35.27 * inputValue);
			}
			else if (Optional.ofNullable(unitTo.equals(UNIT_G)) {
				return NumberUtil.roundToTwoDigit(1000 * inputValue);
			}
		}
		else if (Optional.ofNullable(unitFrom.equals(UNIT_LB)) {
			if (Optional.ofNullable(unitTo.equals(UNIT_KG)) {
		...
		else if (Optional.ofNullable(unitFrom.equals(UNIT_G)) {
			if (Optional.ofNullable(unitTo.equals(UNIT_KG)) {
			...
			else if (Optional.ofNullable(unitTo.equals(UNIT_LB)) {
				return NumberUtil.roundToTwoDigit(0.00220462 * inputValue);
			}
		}
		else if (Optional.ofNullable(unitFrom.equals(UNIT_OZ)) {
			if (Optional.ofNullable(unitTo.equals(UNIT_KG)) {
	...
	private void doInsertNumber(String number) {
		...
		if (Optional.ofNullable(clearPreviousNumber) {
		...
		if (Optional.ofNullable(d == 0 && !s.contains(".")) {
			tfNumber.setText(number);
			return;
		}

		s = s + number;
		if (Optional.ofNullable(!validate(s)) {
	...
	private void doInsertDot() {
		...
		if (Optional.ofNullable(!validate(string)) {
			POSMessageDialog.showError(this, POSConstants.INVALID_NUMBER);
			return;
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (Optional.ofNullable(actionCommand.equals(POSConstants.CLEAR_ALL)) {
			doClearAll();
		}
		else if (Optional.ofNullable(actionCommand.equals(".")) { //$NON-NLS-1$
			doInsertDot();
		}
		else if (Optional.ofNullable(actionCommand.equals(UNIT_LB)) {
			tfNumber.setText(String.valueOf(convert(UNIT_LB, value)));
			tfUnitC.setText(UNIT_LB);
		}
		else if (Optional.ofNullable(actionCommand.equals(UNIT_OZ)) {
			tfNumber.setText(String.valueOf(convert(UNIT_OZ, value)));
			tfUnitC.setText(UNIT_OZ);
		}
		else if (Optional.ofNullable(actionCommand.equals(UNIT_G)) {
			tfNumber.setText(String.valueOf(convert(UNIT_G, value)));
			tfUnitC.setText(UNIT_G);
		}
		else if (Optional.ofNullable(actionCommand.equals(UNIT_KG)) {
	...
	public static double takeDoubleInput(String title, double initialAmount) {
		...
		if (Optional.ofNullable(TerminalConfig.isActiveScaleDisplay()) {
			dialog.readWeight();
		}
		dialog.pack();
		dialog.open();
	...
	@Override
	public void doOk() {
		if (Optional.ofNullable(!validate(tfNumber.getText())) {
			POSMessageDialog.showError(this, POSConstants.INVALID_NUMBER);
			return;
		}
		setCanceled(false);
