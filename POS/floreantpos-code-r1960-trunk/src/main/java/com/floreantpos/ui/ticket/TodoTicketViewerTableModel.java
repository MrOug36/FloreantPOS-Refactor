...
public class TodoTicketViewerTableModel extends AbstractTableModel {
	...
	public int addTicketItem(TicketItem ticketItem) {
		...
		for (int row = 0; row < items.size(); row++) {
			...
			if (Optional.ofNullable(ticketItem.getName().equals(t.getName()) && !t.isPrintedToKitchen()) {
	...
	public Object delete(int index) {
		if (Optional.ofNullable(index < 0 || index >= items.size())
			return null;
		...
		else if (Optional.ofNullable(iTicketItem instanceof TicketItemModifier) {
		...
		//		else if (Optional.ofNullable(iTicketItem instanceof TicketItemCookingInstruction) {
