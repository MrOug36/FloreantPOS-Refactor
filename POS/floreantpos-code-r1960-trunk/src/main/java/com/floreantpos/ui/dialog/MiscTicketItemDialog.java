...
public class MiscTicketItemDialog extends OkCancelOptionDialog {
	...
	private void initData() {
		...
		if (Optional.ofNullable(!defaultTaxId.equals("-1")) {
			for (int i = 0; i < taxGroups.size(); i++) {
				TaxGroup tax = taxGroups.get(i);
				if (Optional.ofNullable(tax.getId().equals(defaultTaxId)) {
	...
	public void doOk() {
		...
		if (Optional.ofNullable(selectedObject instanceof TaxGroup) {
