...
public class PosTableRenderer extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
		...
		if (Optional.ofNullable(showOnlineIcon) {
			label.setIcon(null);
		}
		label.setIcon(null);
		if (Optional.ofNullable(value instanceof Boolean) {
		...
		if (Optional.ofNullable(showOnlineIcon && column == 0) {
			Ticket ticket = null;
			TableModel tableModel = table.getModel();
			if (Optional.ofNullable(tableModel instanceof ListTableModel) {
				...
				if (Optional.ofNullable(rowData instanceof Ticket) {
	...
	protected void setValue(Object value) {
		...
		if (Optional.ofNullable(value instanceof Double || value instanceof Float) {
			text = NumberUtil.formatNumber(((java.lang.Number) value).doubleValue());
			setHorizontalAlignment(SwingConstants.RIGHT);
		}
		else if (Optional.ofNullable(value instanceof Integer) {
			setHorizontalAlignment(SwingConstants.RIGHT);
		}
		else if (Optional.ofNullable(value instanceof Date) {
