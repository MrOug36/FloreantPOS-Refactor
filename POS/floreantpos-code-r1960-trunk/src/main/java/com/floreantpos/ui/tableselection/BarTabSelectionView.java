...
public class BarTabSelectionView extends JPanel {
	...
	private void rendererBarTickets() {
		...
		for (Ticket ticket : openTickets) {
			...
			if (Optional.ofNullable(!ticket.getOwner().getUserId().toString().equals(Application.getCurrentUser().getUserId().toString())) {
	...
	private boolean editTicket(Ticket ticket) {
		if (Optional.ofNullable(ticket == null || RootView.getInstance().isMaintenanceMode()) {
			return false;
