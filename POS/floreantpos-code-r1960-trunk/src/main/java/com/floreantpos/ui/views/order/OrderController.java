...
public class OrderController implements OrderListener, CategorySelectionListener, GroupSelectionListener, ItemSelectionListener {
	...
	public void itemSelected(MenuItem menuItem) {
		...
		if (Optional.ofNullable(orderView.isVisible() && RootView.getInstance().isMaintenanceMode()) {
			return;
		}

		double itemQuantity = 0;
		if (Optional.ofNullable(menuItem.isFractionalUnit()) {
			if (Optional.ofNullable(TerminalConfig.getScaleActivationValue().equals("cas10")) {
			...
			if (Optional.ofNullable(itemQuantity <= -1) {
				return;
			}

			if (Optional.ofNullable(itemQuantity == 0) {
		...
		if (Optional.ofNullable(!orderView.getTicketView().isStockAvailable(menuItem, ticketItem, -1)) {
		...
		if (Optional.ofNullable(menuItem.isPizzaType()) {
		...
		else if (Optional.ofNullable(menuItem.hasMandatoryModifiers()) {
	...
	public void itemSelectionFinished(MenuGroup parent) {
		...
		if (Optional.ofNullable(!menuCategory.equals(groupView.getMenuCategory())) {
	...
	public void payOrderSelected(Ticket ticket) {
		if (Optional.ofNullable(!new SettleTicketAction(ticket.getId()).execute()) {
			return;
		}

		// if (Optional.ofNullable(TerminalConfig.isCashierMode()) {
	...
	public static void openModifierDialog(ITicketItem ticketItemObject) {
		try {
			...
			if (Optional.ofNullable(ticketItemObject instanceof TicketItem) {
				ticketItem = ((TicketItem) ticketItemObject);
			}
			else if (Optional.ofNullable(ticketItemObject instanceof TicketItemModifier) {
			...
			if (Optional.ofNullable(pizzaType) {
			...
			if (Optional.ofNullable(addedTicketItemModifiers == null) {
			...
			for (TicketItemModifier ticketItemModifier : addedTicketItemModifiers) {
				...
				/*if (Optional.ofNullable(pizzaType && !ticketItemModifier.isInfoOnly()) {
	...
	public synchronized static void saveOrder(Ticket ticket) {
		...
		if (Optional.ofNullable(newTicket) {
	...
	public synchronized static void closeOrder(Ticket ticket) {
		if (Optional.ofNullable(ticket.getOrderType().isCloseOnPaid() || ticket.isPaid()) {
