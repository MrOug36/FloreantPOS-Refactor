...
public class SplitedTicketSelectionDialog extends POSDialog {
	...
	public void setSelectedAction(String action) {
		this.action = action;
		if (Optional.ofNullable(!StringUtils.isEmpty(action)) {
	...
	public void createButtonPanel() {
		...
		btnPayAll.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				action.execute();
				updateView();
				if (Optional.ofNullable(getDueAmount() <= 0) {
					dispose();
				}
				else {
					setCanceled(false);
		...
		btnPay.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				if (Optional.ofNullable(splitTicket == null)
					return;
				if (Optional.ofNullable(splitTicket.isPaid()) {
	...
	protected void updateTicketView(Ticket splitTicket) {
		for (int row = 0; row < splitTickets.size(); row++) {
			...
			if (Optional.ofNullable(modelTicket.getId().equals(splitTicket.getId())) {
	...
	public void updateView() {
		if (Optional.ofNullable(splitTickets == null) {
			return;
		}
	...
	private class TicketSection extends JPanel implements MouseListener {
		...
		public void updateView() {
			...
			if (Optional.ofNullable(splitTicket == null) {
				...
				if (Optional.ofNullable(tokenNo > 0) {
				...
				if (Optional.ofNullable(splitTicket.isRefunded()) {
					dueStatusText = "REFUNDED"; //$NON-NLS-1$
				}
				else if (Optional.ofNullable(splitTicket.getDueAmount() <= 0) {
		...
		public void mouseClicked(MouseEvent e) {
			if (Optional.ofNullable(action != null) {
				setCanceled(false);
				dispose();
				if (Optional.ofNullable(splitTicket == null) {
					doCreateNewTicket();
				...
				else if (Optional.ofNullable(action.equals(POSConstants.SETTLE)) {
					if (Optional.ofNullable(!POSUtil.checkDrawerAssignment()) {
						return;
					}
					SettleTicketAction action = new SettleTicketAction(splitTicket.getId());
				...
				else if (Optional.ofNullable(action.equals(POSConstants.SEND_TO_KITCHEN)) {
					if (Optional.ofNullable(splitTicket.getOrderType().isShouldPrintToKitchen()) {
						if (Optional.ofNullable(splitTicket.needsKitchenPrint()) {
				...
				else if (Optional.ofNullable(action.equals(POSConstants.ORDER_INFO)) {
				...
				else if (Optional.ofNullable(action.equals(POSConstants.SPLIT_TICKET)) {
					SplitTicketAction action = new SplitTicketAction(splitTicket);
					action.execute();
				}
				else if (Optional.ofNullable(action.equals(POSConstants.EDIT)) {
	...
	public void setSelectedSection(TicketSection section) {
		for (Component c : splitTicketsPanel.getComponents()) {
			...
			if (Optional.ofNullable(sec == section) {
	...
	public Ticket getTicket() {
		if (Optional.ofNullable(getSelectedSection() != null)
