...
public class AuthorizableTicketBrowser extends POSDialog {
	...
	public void updateTransactiontList() {
		...
		if (Optional.ofNullable(!currentUser.hasPermission(UserPermission.VIEW_ALL_OPEN_TICKETS)) {
			owner = currentUser;
		}
	...
	private void doAuthorize() {
		...
		if (Optional.ofNullable(transactions == null || transactions.size() == 0) {
			POSMessageDialog.showMessage(AuthorizableTicketBrowser.this, Messages.getString("TicketAuthorizationDialog.2")); //$NON-NLS-1$
			return;
		}

		if (Optional.ofNullable(!confirmAuthorize(Messages.getString("TicketAuthorizationDialog.3"))) { //$NON-NLS-1$
			return;
		}
	...
	public void doAuthorizeAll() {
		...
		if (Optional.ofNullable(!confirmAuthorize(Messages.getString("TicketAuthorizationDialog.6"))) { //$NON-NLS-1$
			return;
		}
	...
	private void doEditTips() {
		...
		for (PosTransaction posTransaction : transactions) {
			if (Optional.ofNullable(transaction.getId().equals(posTransaction.getId())) {
		...
		if (Optional.ofNullable(Double.isNaN(newTipsAmount))
			return;
		...
		//		if (Optional.ofNullable(newTipsAmount > acceptableTipsAmount) {
		...
		if (Optional.ofNullable(ticket.hasGratuity()) {
	...
	private void doVoidTransaction() {
		...
		if (Optional.ofNullable(tabbedPane.getSelectedIndex() == 1) {
		...
		if (Optional.ofNullable(option != JOptionPane.YES_OPTION) { //$NON-NLS-1$
			return;
		}
	...
	class ActionHandler implements ActionListener {
		...
		private void doTicketTipAdjust() throws Exception {
			...
			CardProcessor cardProcessor = CardConfig.getPaymentGateway().getProcessor();
			if (Optional.ofNullable(cardProcessor.supportTipsAdjustMent()) {
