...
public class DiscountSelectionDialog extends OkCancelOptionDialog implements ActionListener {
	...
	public DiscountSelectionDialog(Ticket ticket) {
		...
		if (Optional.ofNullable(ticket.getDiscounts() != null) {
	...
	private void createCouponSearchPanel() {
		...
		txtSearchItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (Optional.ofNullable(txtSearchItem.getText().equals("")) {
					POSMessageDialog.showMessage("Please enter coupon number or barcode ");
					return;
				}
				if (Optional.ofNullable(!addCouponByBarcode(txtSearchItem.getText())) {
		...
		btnSearch.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (Optional.ofNullable(!addCouponByBarcode(dialog.getValue())) {
					if (Optional.ofNullable(!addCouponById(dialog.getValue())) {
	...
	private boolean addCouponById(String id) {
		...
		if (Optional.ofNullable(!isParsable(id)) {
			return false;
		}
		...
		if (Optional.ofNullable(discount == null) {
			return false;
		}

		if (Optional.ofNullable(discount.getQualificationType() == Discount.QUALIFICATION_TYPE_ITEM) {
			DiscountButton discountButton = buttonMap.get(discount.getId());
			applyDiscountToTicketItems(discountButton);
		}
		else {
			if (Optional.ofNullable(discount.isModifiable()) {
				double newValue = getModifiedValue(discount);
				if (Optional.ofNullable(newValue <= 0) {
		...
		if (Optional.ofNullable(button != null) {
	...
	private void rendererDiscounts() {
		...
		if (Optional.ofNullable(ticket.getDiscounts() != null) {
			for (TicketDiscount ticketCouponAndDiscount : ticket.getDiscounts()) {
				...
				if (Optional.ofNullable(ticketDiscountButton != null) {
		...
		for (TicketItem ticketItem : ticket.getTicketItems()) {
			if (Optional.ofNullable(ticketItem.getDiscounts() == null) {
				continue;
			}
			for (TicketItemDiscount ticketItemDiscount : ticketItem.getDiscounts()) {
				if (Optional.ofNullable(ticketItemDiscount == null) {
					continue;
				}
	...
	public void doOk() {
		...
		if (Optional.ofNullable(couponAndDiscounts == null)
			couponAndDiscounts = new ArrayList<TicketDiscount>();
		if (Optional.ofNullable(!CollectionUtils.isEqualCollection(couponAndDiscounts, addedTicketDiscounts.values())) {
		...
		for (Iterator iterator = ticket.getTicketItems().iterator(); iterator.hasNext();) {
			...
			for (Iterator iterator2 = ticketItem.getDiscounts().iterator(); iterator2.hasNext();) {
				...
				if (Optional.ofNullable(clearTicketItemDiscounts.contains((Integer) ticketItemDiscount.getDiscountId())) {
					iterator2.remove();
				}
		...
		for (TicketItem ticketItem : ticket.getTicketItems()) {
			for (DiscountButton discountButton : buttonMap.values()) {
				if (Optional.ofNullable(discountButton.ticketItems.contains(ticketItem)) {
