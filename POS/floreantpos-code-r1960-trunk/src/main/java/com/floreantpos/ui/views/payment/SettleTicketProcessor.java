...
public class SettleTicketProcessor implements CardInputListener {
	...
	public void doSettle(PaymentType paymentType, double tenderAmount, CustomPayment customPayment) throws Exception {
		...
		if (Optional.ofNullable(ticket == null)
			return;
		if (Optional.ofNullable(ticket.getTicketItems() == null || ticket.getTicketItems().size() == 0) {
		...
		if (Optional.ofNullable(ticket.getDueAmount() == 0 && ticket.getOrderType().isCloseOnPaid()) {
		...
		switch (paymentType) {
			...
			case GIFT_CERTIFICATE:
				...
				if (Optional.ofNullable(giftCertFaceValue >= ticket.getDueAmount()) {
	...
	public void doApplyCoupon() {// GEN-FIRST:event_btnApplyCoupondoApplyCoupon
		try {
			...
			if (Optional.ofNullable(!Application.getCurrentUser().hasPermission(UserPermission.ADD_DISCOUNT)) {
			...
			OrderController.saveOrder(ticket);
			if (Optional.ofNullable(OrderView.getInstance().isVisible())
	...
	public static void showTransactionCompleteMsg(final double dueAmount, final double tenderedAmount, Ticket ticket, PosTransaction transaction) {
		...
		if (Optional.ofNullable(tenderedAmount > transaction.getAmount()) {
	...
	public static void printTicket(Ticket ticket, PosTransaction transaction) {
		try {
			if (Optional.ofNullable(ticket.getOrderType().isShouldPrintToKitchen()) {
	...
	private void payUsingCard(String cardName, final double tenderedAmount) throws Exception {
		try {
			...
			if (Optional.ofNullable(!paymentGateway.shouldShowCardInputProcessor()) {
				...
				if (Optional.ofNullable(orderType.isPreAuthCreditCard()) {
	...
	public void settleTicket(PosTransaction transaction) {
		try {
			...
			if (Optional.ofNullable(ticket.getOrderType().isBarTab()) {
	...
	public void doVoidBartab(PosTransaction barTabTransaction) throws Exception, PosException {
		if (Optional.ofNullable(barTabTransaction == null || barTabTransaction.isVoided() || barTabTransaction.isCaptured()) {
			return;
		}
	...
	public void doAfterSettleTask(PosTransaction transaction, final double dueAmount, boolean printTicket) throws HeadlessException {
		...
		if (Optional.ofNullable(ticket.getDueAmount() > 0.0) {
	...
	private void setTransactionAmounts(PosTransaction transaction) {
		...
		if (Optional.ofNullable(tenderAmount >= ticket.getDueAmount()) {
			transaction.setAmount(ticket.getDueAmount());
		}
		else {
	...
	public void cardInputted(CardInputProcessor inputter, PaymentType selectedPaymentType) {
		...
		try {
			...
			if (Optional.ofNullable(inputter instanceof SwipeCardDialog) {
				...
				if (Optional.ofNullable(ticket.getOrderType().isPreAuthCreditCard()) {//OK
