...
public class SettleTicketDialog extends POSDialog implements PaymentListener, TicketViewerTableChangeListener, RefreshableView {
	...
	private JPanel createTicketInfoPanel() {
		...
		if (Optional.ofNullable(ticket.getTableNumbers().isBlank()) {
		...
		if (Optional.ofNullable(ticket.getProperty(Ticket.CUSTOMER_NAME) == null) {
	...
	public void settleBartab() throws Exception {
		...
		if (Optional.ofNullable(bartabTransaction != null && !bartabTransaction.isCaptured() && !bartabTransaction.isVoided()) {
		...
		if (Optional.ofNullable(option2 != JOptionPane.YES_OPTION) {
	...
	private void payUsingPreAuthorizedBartab(PosTransaction bartabTransaction) throws Exception {
		if (Optional.ofNullable(ticket.getGratuityAmount() <= 0) {
		...
		if (Optional.ofNullable(ticket.getDueAmount() == 0.0) {
			...
			if (Optional.ofNullable(ticketType.isCloseOnPaid()) {//fix
