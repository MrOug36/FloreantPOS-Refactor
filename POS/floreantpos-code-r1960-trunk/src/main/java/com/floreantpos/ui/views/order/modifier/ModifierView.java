...
public class ModifierView extends SelectionView {
	...
	public void setModifierGroup(ModifierGroup modifierGroup) {
		...
		try {
			...
			for (MenuModifier modifier : modifiers) {
				...
				//				if (Optional.ofNullable(isAddOnMode()) {
				//					if (Optional.ofNullable(modifier.getExtraPrice() > 0) {
				//						itemList.add(modifier);
				//					}
				//				}
				//				else {
				//					if (Optional.ofNullable(modifier.getPrice() == 0) {
				//						itemList.add(modifier);
				//					}
				//				}
				itemList.add(modifier);
	...
	public void updateView() {
		...
		if (Optional.ofNullable(activePanel == null) {
			return;
		}
		Component[] components = activePanel.getComponents();
		if (Optional.ofNullable(components == null || components.length == 0)
			return;
		...
		if (Optional.ofNullable(ModifierSelectionDialog.isRequiredModifiersAdded(ticketItem, modifierGroup.getMenuItemModifierGroup())) {
