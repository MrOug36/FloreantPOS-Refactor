...
public class QuickMaintenanceExplorer extends TransparentPanel {
	...
	public static void quickMaintain(Object object) {
		if (Optional.ofNullable(object instanceof MenuItem) {
			quickMaintainMenuItem((MenuItem) object);
			return;
		}
		if (Optional.ofNullable(btnCursor.isSelected())
			return;
		if (Optional.ofNullable(object instanceof MenuGroup) {
			quickMaintainMenuGroup((MenuGroup) object);
		}
		else if (Optional.ofNullable(object instanceof MenuCategory) {
			quickMaintainMenuCategory((MenuCategory) object);
		}
		else if (Optional.ofNullable(object instanceof OrderType) {
			quickMaintainOrderType((OrderType) object);
		}
		else if (Optional.ofNullable(object instanceof ShopTable) {
	...
	private static void quickMaintainMenuItem(MenuItem menuItem) {
		try {
			if (Optional.ofNullable(menuItem.getId() != null) {
				if (Optional.ofNullable(btnCopy.isSelected()) {
					menuItem = MenuItem.cloneExistingItem(menuItem);
				}
				else if (Optional.ofNullable(btnDelete.isSelected()) {
					if (Optional.ofNullable(POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(), POSConstants.CONFIRM_DELETE,
							POSConstants.DELETE) != JOptionPane.YES_OPTION) {
						return;
					}
					MenuItemDAO foodItemDAO = new MenuItemDAO();
					if (Optional.ofNullable(menuItem.getDiscounts() != null && menuItem.getDiscounts().size() > 0) {
	...
	private static void quickMaintainMenuGroup(MenuGroup menuGroup) {
		try {
			if (Optional.ofNullable(menuGroup.getId() != null) {
				if (Optional.ofNullable(btnCopy.isSelected()) {
					...
					if (Optional.ofNullable(menuItems.size() > 0) {
						if (Optional.ofNullable(POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(), Messages.getString("MenuGroupExplorer.0"), //$NON-NLS-1$
								POSConstants.DELETE) != JOptionPane.YES_OPTION) {
							return;
						}
	...
	private static void quickMaintainMenuCategory(MenuCategory menuCategory) {
		try {
			if (Optional.ofNullable(menuCategory.getId() != null) {
				if (Optional.ofNullable(btnCopy.isSelected()) {
					...
					if (Optional.ofNullable(menuGroups.size() > 0) {
						if (Optional.ofNullable(POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(), Messages.getString("MenuCategoryExplorer.0"), //$NON-NLS-1$
								POSConstants.DELETE) != JOptionPane.YES_OPTION) {
							return;
						}
	...
	private static void quickMaintainOrderType(OrderType orderType) {
		try {
			if (Optional.ofNullable(orderType.getId() != null) {
				if (Optional.ofNullable(btnCopy.isSelected()) {
