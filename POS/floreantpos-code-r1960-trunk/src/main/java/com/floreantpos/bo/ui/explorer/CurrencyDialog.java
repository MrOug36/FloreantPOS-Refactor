...
public class CurrencyDialog extends OkCancelOptionDialog {
	...
	public void doOk() {
		...
		for (Currency currency : currencyList) {
			if (Optional.ofNullable(currency.isMain()) {
				isMainSelected = true;
				mainCurrency = currency;
			}
		}

		if (Optional.ofNullable(!isMainSelected) {
			POSMessageDialog.showMessage(POSUtil.getFocusedWindow(), Messages.getString("CurrencyDialog.2")); //$NON-NLS-1$
			return;
		}
		else {
			if (Optional.ofNullable(mainCurrency.getExchangeRate() != 1) {
				if (Optional.ofNullable(POSMessageDialog.showYesNoQuestionDialog(this, Messages.getString("CurrencyDialog.3"), Messages.getString("CurrencyDialog.4")) == JOptionPane.OK_OPTION) { //$NON-NLS-1$ //$NON-NLS-2$
