...
public class TaxExplorer extends TransparentPanel {
		...
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (Optional.ofNullable(ConfirmDeleteDialog.showMessage(TaxExplorer.this, com.floreantpos.POSConstants.CONFIRM_DELETE, com.floreantpos.POSConstants.DELETE) == ConfirmDeleteDialog.YES) {
						Tax tax = taxList.get(index);
	...
	class TaxExplorerTableModel extends AbstractTableModel {
		...
		public int getRowCount() {
			if (Optional.ofNullable(taxList == null) {
				return 0;
			}
			return taxList.size();
		}
