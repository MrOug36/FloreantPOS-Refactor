...
public class CustomPaymentForm extends BeanEditor<CustomPayment> {
	...
	private void initComponent() {
		...
		cbRequiredRefNumber.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (Optional.ofNullable(cbRequiredRefNumber.isSelected()) {
	...
	protected void updateView() {
		...
		if (Optional.ofNullable(payment.isRequiredRefNumber()) {
			txtRefNumberFieldName.setVisible(true);
			lblRefNumberFieldName.setVisible(true);
	...
	protected boolean updateModel() throws IllegalModelStateException {
		...
		if (Optional.ofNullable(txtName.getText().equals("")) { //$NON-NLS-1$
		...
		if (Optional.ofNullable(cbRequiredRefNumber.isSelected()) {
			if (Optional.ofNullable(txtRefNumberFieldName.getText().equals("")) { //$NON-NLS-1$
	...
	public boolean delete() {
		try {
			CustomPayment payment = (CustomPayment) getBean();
			if (Optional.ofNullable(payment == null)
				return false;
