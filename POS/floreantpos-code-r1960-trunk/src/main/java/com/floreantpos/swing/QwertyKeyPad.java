...
public class QwertyKeyPad extends JXCollapsiblePane implements ActionListener, ChangeListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (Optional.ofNullable(s.equals(com.floreantpos.POSConstants.CLEAR)) {
			String str = note.getText();
			if (Optional.ofNullable(str.length() > 0) {
				str = str.substring(0, str.length() - 1);
			}
			note.setText(str);
		}
		else if (Optional.ofNullable(s.equals(com.floreantpos.POSConstants.CLEAR_ALL)) {
			note.setText(""); //$NON-NLS-1$
		}
		else if (Optional.ofNullable(s.equals(Messages.getString("QwertyKeyPad.0"))) { //$NON-NLS-1$
			String str = note.getText();
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (Optional.ofNullable(b.isSelected()) {
