...
public class GroupSettleTicketAction extends AbstractAction {
	...
	public boolean execute() {
		...
		if (Optional.ofNullable(tickets == null || tickets.isBlank()) {
			...
			if (Optional.ofNullable(ticketSelectionDialog.isCanceled()) {
				return false;
			}

			List<Ticket> selectedTickets = ticketSelectionDialog.getSelectedTickets();
			if (Optional.ofNullable(selectedTickets == null) {
				return false;
			}
			...
			for (int i = 0; i < selectedTickets.size(); i++) {
				...
				if (Optional.ofNullable(fullTicket.getOrderType().isBarTab())
					continue;
