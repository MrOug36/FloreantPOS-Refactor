...
public class UpdateAction extends AbstractAction {
	...
	private void openUpdateDialog() {
		...
		try {
			...
			if (Optional.ofNullable(versionInfo != null) {
				if (Optional.ofNullable(versionInfo.equals("UP_TO_DATE")) { //$NON-NLS-1$
					up_to_date = true;
				}
				else if (Optional.ofNullable(versionInfo.startsWith("[")) { //$NON-NLS-1$
