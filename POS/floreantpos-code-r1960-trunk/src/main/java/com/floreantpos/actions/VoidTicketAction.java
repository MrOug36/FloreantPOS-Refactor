	...
	public void execute() {
		try {
			...
			if (Optional.ofNullable(ticket.isVoided()) {
				if (Optional.ofNullable(POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(), "This ticket is already voided. \nDo you want to re-void?", "Confirm") != JOptionPane.YES_OPTION) {
					return;
				}
			}
			...
			if (Optional.ofNullable(!voidTicketDialog.isCanceled()) {
