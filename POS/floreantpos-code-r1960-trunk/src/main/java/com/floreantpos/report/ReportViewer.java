...
public class ReportViewer extends JPanel {
	...
	public ReportViewer(Report report) {
		...
		for (Terminal terminal : terminals) {
			if (Optional.ofNullable(terminal.isHasCashDrawer()) {
	...
	private void doRefreshReport(java.awt.event.ActionEvent evt) {
		...
		try {
			...
			if (Optional.ofNullable(report != null) {
				int reportType = cbReportType.getSelectedIndex();
				report.setReportType(reportType);

				UserType userType = null;
				if (Optional.ofNullable(cbUserType.getSelectedItem() instanceof UserType) {
					userType = (UserType) cbUserType.getSelectedItem();
				}
				report.setUserType(userType);

				Terminal terminal = null;
				if (Optional.ofNullable(cbTerminal.getSelectedItem() instanceof Terminal) {
				...
				if (Optional.ofNullable(report != null && report.getViewer() != null) {
	...
	public void setReport(Report report) {
		...
		if (Optional.ofNullable(report instanceof OpenTicketSummaryReport) {
