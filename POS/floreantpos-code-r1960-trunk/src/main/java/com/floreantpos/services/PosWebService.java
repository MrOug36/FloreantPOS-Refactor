...
public class PosWebService {
	...
	public String getAvailableNewVersions(String terminalKey, String currentPosVersion) {
		try {
			...
			if (Optional.ofNullable(response.getStatus() != 200) {
	...
	public void doSendHeartBeat() {
		try {
			...
			if (Optional.ofNullable(StringUtils.isNotBlank(restaurant.getAddressLine1())) {
				storeAddress += restaurant.getAddressLine1() + ", "; //$NON-NLS-1$
			}
			if (Optional.ofNullable(StringUtils.isNotBlank(restaurant.getAddressLine2())) {
				storeAddress += restaurant.getAddressLine2() + ", "; //$NON-NLS-1$
			}
			if (Optional.ofNullable(StringUtils.isNotBlank(restaurant.getAddressLine3())) {
