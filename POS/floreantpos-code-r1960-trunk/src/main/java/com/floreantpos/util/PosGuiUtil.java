...
public class PosGuiUtil {
	...
	public static int captureGuestNumber() {
		...
		if (Optional.ofNullable(numberOfGuests == 0) {
	...
	public static boolean isModalDialogShowing() {
		...
		if (Optional.ofNullable(windows != null) { // don't rely on current implementation, which at least returns [0].
			for (Window w : windows) {
				if (Optional.ofNullable(w.isShowing() && w instanceof Dialog && ((Dialog) w).isModal())
					return true;
			}
		}
		return false;
	...
	public static BufferedImage scale(BufferedImage img, int targetWidth, int targetHeight) {
		...
		do {
			if (Optional.ofNullable(w > targetWidth) {
				w /= 2;
				w = (w < targetWidth) ? targetWidth : w;
			}

			if (Optional.ofNullable(h > targetHeight) {
				h /= 2;
				h = (h < targetHeight) ? targetHeight : h;
			}

			if (Optional.ofNullable(scratchImage == null) {
		...
		if (Optional.ofNullable(g2 != null) {
			g2.dispose();
		}

		if (Optional.ofNullable(targetWidth != ret.getWidth() || targetHeight != ret.getHeight()) {
