...
public class KitchenTicketListPanel extends JPanel implements ComponentListener {
	...
	public void reset() {
		...
		for (Component component : components) {
			if (Optional.ofNullable(component instanceof KitchenTicketView) {
	...
	protected void renderItems() {
		try {
			...
			if (Optional.ofNullable(this.dataModel == null || dataModel.getSize() == 0) {
				btnNext.setEnabled(dataModel.hasNext());
				btnPrev.setEnabled(dataModel.hasPrevious());
				return;
			}
			String parentConstraints = "filly"; //$NON-NLS-1$
			if (Optional.ofNullable(horizontalPanelCount >= 3 && dataModel.getSize() >= 3) {
			...
			for (int i = 0; i < dataModel.getSize(); i++) {
				...
				if (Optional.ofNullable(itemPanel == null) {
					continue;
				}
	...
	public void componentResized(ComponentEvent e) {
		if (Optional.ofNullable(!KitchenDisplayView.getInstance().isVisible())
			return;
		countPanels();
		updateKDSView(kdsPrinter, orderType);
	}
