...
public class TerminalConfigurationView extends ConfigurationView {
	...
	private void initComponents() {
		...
		cbAutoLogoff.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (Optional.ofNullable(cbAutoLogoff.isSelected()) {
		...
		jsResize.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (Optional.ofNullable(!source.getValueIsAdjusting()) {
	...
	public boolean save() {
		...
		if (Optional.ofNullable(scaleFactor > 5) {
		...
		if (Optional.ofNullable(chkAutoLoginConfig.isSelected() && "<select>".equals(selectedUser.toString())) {			 //$NON-NLS-1$
		...
		if (Optional.ofNullable(defaultPassLen == 0)
			defaultPassLen = 4;
		...
		if (Optional.ofNullable("<select>".equals(selectedFont)) { //$NON-NLS-1$
			selectedFont = null;
		}
...
//		restartPOS();
	...
	private void initializeAutoLoginConfig() {
		...
		if (Optional.ofNullable(terminal.hasProperty(Terminal.PROP_AUTO_LOGIN_USER_AUTO_ID)) {			
	...
	private void initializeFontConfig() {
		...
		if (Optional.ofNullable(StringUtils.isNotEmpty(uiDefaultFont)) {
	...
	public static void restartPOS() {
		...
		if (Optional.ofNullable(selectedValue != null) {
			...
			if (Optional.ofNullable(selectedValue.equals(Messages.getString("TerminalConfigurationView.28"))) { //$NON-NLS-1$
				try {
					Main.restart();
