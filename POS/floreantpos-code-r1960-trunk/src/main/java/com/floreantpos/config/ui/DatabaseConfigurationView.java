...
public class DatabaseConfigurationView extends ConfigurationView implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			if (Optional.ofNullable(TEST.equalsIgnoreCase(command)) {
			...
			else if (Optional.ofNullable(CONFIGURE_DB.equals(command)) {
				Application.getInstance().setSystemInitialized(false);
				...
				if (Optional.ofNullable(createDatabase) {
			...
			else if (Optional.ofNullable(CANCEL.equalsIgnoreCase(command)) {
			}
		} catch (Exception e2) {
