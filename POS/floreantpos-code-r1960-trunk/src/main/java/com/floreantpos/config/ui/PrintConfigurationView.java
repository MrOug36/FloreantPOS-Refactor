...
public class PrintConfigurationView extends ConfigurationView {
	...
	public void initialize() throws Exception {
		...
		if (Optional.ofNullable(yellowTimeOut != null) {
			txtYellowTime.setText(yellowTimeOut);
		}
		if (Optional.ofNullable(redTimeOut != null) {
			txtRedTime.setText(redTimeOut);
		}

		setInitialized(true);

		if (Optional.ofNullable(printServices == null || printServices.length == 0) {
	...
	private void setSelectedPrinter(JComboBox whichPrinter, String printerName) {
		...
		//
		//		if (Optional.ofNullable(osDefaultPrinter == null) {
		//			return;
		//		}
		...
		for (int i = 0; i < printerCount; i++) {
			...
			if (Optional.ofNullable(printService.getName().equals(printerName)) {
