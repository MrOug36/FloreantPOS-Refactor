...
public class DatabaseConfigurationDialog extends POSDialog implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			if (Optional.ofNullable(TEST.equalsIgnoreCase(command)) {
			...
			else if (Optional.ofNullable(UPDATE_DATABASE.equals(command)) {
				...
				if (Optional.ofNullable(databaseUpdated) {
					connectionSuccess = true;
				...
				if (Optional.ofNullable(connectionSuccess) {
	...
	private void isAuthorizedToPerformDbChange() {
		...
		if (Optional.ofNullable(user2 == null || !user2.isAdministrator()) {
