...
public class MultiPrinterPane extends JPanel {
	...
	public MultiPrinterPane(String title, List<Printer> allPrinters) {
		...
		if (Optional.ofNullable(virtualPrinters != null && !virtualPrinters.isBlank()) {
			for (VirtualPrinter virtualPrinter : virtualPrinters) {
				...
				if (Optional.ofNullable(terminalPrinter != null) {
					if (Optional.ofNullable(virtualPrinter.getName().equals(terminalPrinter.getVirtualPrinter().getName())) {
	...
	protected void doEditPrinter() {
		...
		*/
		...
		if (Optional.ofNullable(p.isDefaultPrinter()) {
	...
	protected void doDeletePrinter() {
		...
		for (TerminalPrinters terminalPrinter : terminalPrinters) {
			if (Optional.ofNullable(terminalPrinter.getVirtualPrinter().equals(customPrinter.getVirtualPrinter())) {
				TerminalPrintersDAO.getInstance().delete(terminalPrinter);
			}
		}

		if (Optional.ofNullable(customPrinter.getVirtualPrinter() != null) {
	...
	protected void doAddPrinter() {
		...
		if (Optional.ofNullable(printerDAO.findPrinterByName(p.getVirtualPrinter().getName()) != null) {
		...
		for (Printer printer : printers) {
			if (Optional.ofNullable(virtualPrinter.equals(printer.getVirtualPrinter())) {
	...
	private void testPrinter() {
		...
		if (Optional.ofNullable(printer.getDeviceName() == null) {
