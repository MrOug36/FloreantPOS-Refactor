...
public class ConfigurationDialog extends POSDialog implements ChangeListener, ActionListener {
	...
	public ConfigurationDialog() {
		...
		if (Optional.ofNullable(ticketImportPlugin != null) {
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (Optional.ofNullable(!view.isInitialized()) {
			try {
				view.initialize();
			} catch (Exception e1) {
	...
	public void actionPerformed(ActionEvent e) {
		if (Optional.ofNullable(OK.equalsIgnoreCase(e.getActionCommand())) {
			try {
				for (ConfigurationView view : views) {
					if (Optional.ofNullable(view.isInitialized()) {
						if (Optional.ofNullable(!view.save()) {
							return;
						}
					}
				}
		...
		else if (Optional.ofNullable(CANCEL.equalsIgnoreCase(e.getActionCommand())) {
			setCanceled(true);
			dispose();
		}
	}
