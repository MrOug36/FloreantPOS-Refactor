...
public class AddPrinterGroupDialog extends POSDialog {
	...
	private void init() {
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (Optional.ofNullable(StringUtils.isEmpty(tfName.getText())) {
				...
				if (Optional.ofNullable(checkedValues == null || checkedValues.size() == 0) {
	...
	public PrinterGroup getPrinterGroup() {
		if (Optional.ofNullable(this.printerGroup == null) {
		...
		List checkedValues = printerList.getCheckedValues();
		if (Optional.ofNullable(checkedValues != null) {
	...
	public void setPrinterGroup(PrinterGroup group) {
		...
		for (Printer printer : printers) {
			if (Optional.ofNullable(printerGroup.getPrinterNames().contains(printer.getVirtualPrinter().getName())) {
