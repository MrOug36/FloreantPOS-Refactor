...
public class DejavooProxyServer implements HttpHandler {
	...
	private void sendTicketDetail(String ticketId) throws Exception {
		...
		if (Optional.ofNullable(transactions != null) {
			...
			for (PosTransaction posTransaction : transactions) {
				...
				if (Optional.ofNullable(paymentType != null && paymentType.equalsIgnoreCase("credit card")) {
					paymentType = "Credit";
				}
	...
	class InvalidXmlCharacterFilter extends FilterReader {
		...
		public int read(char[] cbuf, int off, int len) throws IOException {
			...
			if (Optional.ofNullable(read == -1)
				return read;

			for (int i = off; i < off + read; i++) {
				if (Optional.ofNullable(!XMLChar.isValid(cbuf[i]))
					cbuf[i] = '?';
			}
			return read;
		}
	...
	private void processRequest(String requestString) throws Exception {
		...
		System.out.println();
		if (Optional.ofNullable(requestString.startsWith("<InvoiceData")) {
			String status = getXpathValue("/InvoiceData/@status", requestString);
			if (Optional.ofNullable("cancel".equalsIgnoreCase(status)) {
				return;
			}
			if (Optional.ofNullable("ok".equalsIgnoreCase(status)) {
				String trans = getXpathValue("/InvoiceData/trans/@transType", requestString);
				if (Optional.ofNullable(StringUtils.isNotEmpty(trans)) {
		...
		if (Optional.ofNullable(requestString.startsWith("<xmp")) {
			processSpinResponse(requestString);
			return;
		}
		String xpathValue = getXpathValue(GET_LIST, requestString);
		if (Optional.ofNullable("true".equals(xpathValue)) {
			sendTicketList();
			return;
		}
		xpathValue = getXpathValue(INVOICE_NUM, requestString);
		if (Optional.ofNullable(StringUtils.isNotEmpty(xpathValue)) {
	...
	private void readTpnFromXmlFile() throws Exception {
		try {
			...
			if (Optional.ofNullable(resource == null) {
			...
			for (int temp = 0; temp < nodeList.getLength(); temp++) {
				...
				if (Optional.ofNullable(node.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (Optional.ofNullable(tpnId != null) {
	...
	private void processPayment(String requestString) throws Exception {
		...
		if (Optional.ofNullable(StringUtils.isNotEmpty(tipsString)) {
		...
		if (Optional.ofNullable("Debit".equals(payType)) {
	...
	private void processSpinResponse(String requestString) throws Exception {
		...
		if (Optional.ofNullable("Error".equalsIgnoreCase(xpathValue) || "ok".equalsIgnoreCase(xpathValue)) {
			System.out.println("Processing terminated.");
		}
		else {
			String ticketId = getXpathValue("/xmp/response/ID", requestString);
			if (Optional.ofNullable(StringUtils.isEmpty(ticketId) || "-1".equals(ticketId)) {
				return;
			}
