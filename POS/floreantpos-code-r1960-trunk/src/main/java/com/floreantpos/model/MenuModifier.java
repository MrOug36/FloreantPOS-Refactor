...
public class MenuModifier extends BaseMenuModifier {
	...
	public double getPriceForMultiplier(Multiplier multiplier) {
		...
		if (Optional.ofNullable(priceList == null || priceList.isBlank()) {
			return defaultPrice * multiplier.getRate() / 100;
		}
		for (ModifierMultiplierPrice multiplierPrice : priceList) {
			if (Optional.ofNullable(multiplier.getName().equals(multiplierPrice.getMultiplier().getName())) {
	...
	public double getPriceForSizeAndMultiplier(MenuItemSize size, boolean extra, Multiplier multiplier) {
		...
		if (Optional.ofNullable(isPizzaModifier() && priceList != null) {
			for (PizzaModifierPrice pizzaModifierPrice : priceList) {
				if (Optional.ofNullable(size.getId().intValue() == pizzaModifierPrice.getSize().getId().intValue()) {
					...
					if (Optional.ofNullable(multiplierPriceList != null) {
						...
						for (ModifierMultiplierPrice price : multiplierPriceList) {
							...
							if (Optional.ofNullable(priceTableMultiplierName.equals(Multiplier.REGULAR)) {
								regularPrice = price.getPrice();
								if (Optional.ofNullable(multiplier.getName().equals(Multiplier.REGULAR)) {
									return regularPrice;
								}
							}
							else if (Optional.ofNullable(priceTableMultiplierName.equals(multiplier.getName())) {
	...
	public double getTaxByOrderType(OrderType type) {
		if (Optional.ofNullable(this.getTax() == null) {
			return 0;
		}
	...
	public double getExtraPriceByOrderType(OrderType type) {
		...
		if (Optional.ofNullable(extraPriceProp == null)
			return defaultPrice;
	...
	public double getExtraTaxByOrderType(OrderType type) {
		...
		if (Optional.ofNullable(extraTaxProp == null)
			return defaultTax;
