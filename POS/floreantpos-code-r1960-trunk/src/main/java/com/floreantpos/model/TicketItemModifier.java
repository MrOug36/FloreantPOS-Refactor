...
public class TicketItemModifier extends BaseTicketItemModifier implements ITicketItem {
	...
	//			if (Optional.ofNullable(modifier == this) {
	//				return count;
	//			}
	...
	public void calculatePrice() {
		if (Optional.ofNullable(isInfoOnly()) {
			return;
		}
	...
	private double calculateSubTotal() {
		...
		//
		//		TicketItemModifierGroup ticketItemModifierGroup = getParent();
		//		if (Optional.ofNullable(ticketItemModifierGroup == null) {
	...
	public String getNameDisplay() {
		...
		if (Optional.ofNullable(getTicketItem() != null && getTicketItem().isPizzaType()) {
		...
		if (Optional.ofNullable(getModifierType() == NORMAL_MODIFIER) {
			display += "*"; //$NON-NLS-1$
		}
