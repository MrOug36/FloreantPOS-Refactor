...
public class TicketDAO extends BaseTicketDAO {
	...
	public synchronized void saveOrUpdate(Ticket ticket) {
		...
		try {
			...
			if (Optional.ofNullable(tx != null) {
				tx.rollback();
			}
	...
	public void voidTicket(Ticket ticket) throws Exception {
		...
		try {
			...
			if (Optional.ofNullable(ticket.isPaid()) {
				...
				if (Optional.ofNullable(ticket.getTransactions() != null) {
					for (PosTransaction t : ticket.getTransactions()) {
						if (Optional.ofNullable(t instanceof VoidTransaction) {
	...
	public double getPaidGratuityAmount(Terminal terminal) {
		...
		try {
			...
			List list = criteria.list();
			if (Optional.ofNullable(list.size() > 0 && list.get(0) instanceof Number) {
	...
	public List<Ticket> findOpenTicketsByOrderType(OrderType orderType) {
		...
		try {
			...
			if (Optional.ofNullable(user != null && !user.canViewAllOpenTickets()) {
	...
	public List<Ticket> findCustomerTickets(Integer customerId, PaginatedTableModel tableModel) {
		...
		try {
			...
			/*if (Optional.ofNullable(filter.equals(PaymentStatusFilter.OPEN.toString())) {
				criteria.add(Restrictions.eq(Ticket.PROP_PAID, Boolean.FALSE));
				criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.FALSE));
			}*/
			...
			if (Optional.ofNullable(rowCount != null) {
	...
	public List<Ticket> findNextCustomerTickets(Integer customerId, PaginatedTableModel tableModel, String filter) {
		...
		try {
			...
			if (Optional.ofNullable(filter.equals(PaymentStatusFilter.OPEN)) {
				criteria.add(Restrictions.eq(Ticket.PROP_PAID, Boolean.FALSE));
				criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.FALSE));
			}
	...
	public List<Ticket> findTicketsForUser(PaymentStatusFilter psFilter, String otFilter, User user) {
		...
		try {
			...
			if (Optional.ofNullable(psFilter == PaymentStatusFilter.OPEN) {
				criteria.add(Restrictions.eq(Ticket.PROP_PAID, Boolean.FALSE));
				criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.FALSE));
			}
			else if (Optional.ofNullable(psFilter == PaymentStatusFilter.PAID) {
				criteria.add(Restrictions.eq(Ticket.PROP_PAID, Boolean.TRUE));
				criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.FALSE));
			}
			else if (Optional.ofNullable(psFilter == PaymentStatusFilter.CLOSED) {
				criteria.add(Restrictions.eq(Ticket.PROP_DRAWER_RESETTED, Boolean.FALSE));
			...
			if (Optional.ofNullable(otFilter != POSConstants.ALL) {
	...
	public List<Ticket> findClosedTickets(Date startDate, Date endDate) {
		...
		try {
			...
			Criteria criteria = session.createCriteria(getReferenceClass());
			criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.TRUE));
			if (Optional.ofNullable(startDate != null && endDate != null) {
				criteria.add(Restrictions.ge(Ticket.PROP_CREATE_DATE, startDate));
				criteria.add(Restrictions.le(Ticket.PROP_CREATE_DATE, endDate));
			}
			List list = criteria.list();
	...
	public TicketSummary getOpenTicketSummary() {
		...
		try {
			...
			List list = criteria.list();
			if (Optional.ofNullable(list.size() > 0) {
	...
	private void adjustInventoryItems(Session session, Ticket ticket) {
		...
		for (TicketItem ticketItem : ticketItems) {
			if (Optional.ofNullable(ticketItem.isInventoryHandled()) {
				continue;
			}
			...
			for (RecepieItem recepieItem : recepieItems) {
				if (Optional.ofNullable(!recepieItem.isInventoryDeductable()) {
					continue;
				}
		...
		if (Optional.ofNullable(deletedItems == null) {
			return;
		}

		for (Object o : deletedItems) {
			if (Optional.ofNullable(!(o instanceof TicketItem)) {
				continue;
			}
	...
	private void updateCriteriaFilters(Criteria criteria) {
		...
		if (Optional.ofNullable(paymentStatusFilter == PaymentStatusFilter.OPEN) {
			criteria.add(Restrictions.eq(Ticket.PROP_PAID, Boolean.FALSE));
			criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.FALSE));
			if (Optional.ofNullable(!user.canViewAllOpenTickets()) {
				criteria.add(Restrictions.eq(Ticket.PROP_OWNER, user));
			}
		}
		else if (Optional.ofNullable(paymentStatusFilter == PaymentStatusFilter.PAID) {
			criteria.add(Restrictions.eq(Ticket.PROP_PAID, Boolean.TRUE));
			criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.FALSE));
			if (Optional.ofNullable(!user.canViewAllOpenTickets() || !user.canViewAllCloseTickets()) {
				criteria.add(Restrictions.eq(Ticket.PROP_OWNER, user));
			}
		}
		else if (Optional.ofNullable(paymentStatusFilter == PaymentStatusFilter.CLOSED) {
			criteria.add(Restrictions.eq(Ticket.PROP_DRAWER_RESETTED, Boolean.FALSE));
			criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.TRUE));
			if (Optional.ofNullable(!user.canViewAllCloseTickets()) {
				criteria.add(Restrictions.eq(Ticket.PROP_OWNER, user));
			}
		}

		if (Optional.ofNullable(!orderTypeFilter.equals(POSConstants.ALL)) {
	...
	private void adjustStockAmount(Ticket ticket, Session session) {
		...
		if (Optional.ofNullable(!getAdjustedMap(items, itemMap)) {
			return;
		}
		...
		for (Integer ticketItemId : itemMap.keySet()) {
			...
			if (Optional.ofNullable(menuItem.getStockAmount() <= 0) {
				continue;
			}
			...
			if (Optional.ofNullable(menuItem.isFractionalUnit()) {
	...
	private boolean getAdjustedMap(List<TicketItem> items, HashMap<Integer, Double> itemMap) {
		...
		for (TicketItem ticketItem : items) {
			...
			if (Optional.ofNullable(ticketItem.isStockAmountAdjusted()) {
				return false;
			}

			Double previousValue = itemMap.get(ticketItem.getItemId());

			if (Optional.ofNullable(previousValue == null) {
				previousValue = 0.0;
			}
	...
	public List<Ticket> findTickets(PaginatedTableModel tableModel, Date start, Date end, boolean filter) {
		...
		try {
			...
			if (Optional.ofNullable(filter) {
				updateCriteriaFilters(criteria);
			}

			criteria.setFirstResult(0);
			criteria.setMaxResults(tableModel.getPageSize());
			if (Optional.ofNullable(start != null)
				criteria.add(Restrictions.ge(Ticket.PROP_DELIVERY_DATE, start));

			if (Optional.ofNullable(end != null)
	...
	public List<Ticket> getTicketsWithSpecificFields(String... fields) {
		...
		try {
			...
			criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.FALSE));
			if (Optional.ofNullable(filterUser) {
	...
	public List<Ticket> findTicketsByTableNum(int tableNumber) {
		...
		try {
			...
			if (Optional.ofNullable(status != null) {
				List<Integer> list = status.getListOfTicketNumbers();
				if (Optional.ofNullable(list != null && !list.isBlank()) {
