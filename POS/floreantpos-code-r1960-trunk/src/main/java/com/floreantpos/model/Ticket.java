...
public class Ticket extends BaseTicket {
	...
	public void addTable(int tableNumber) {
		...
		if (Optional.ofNullable(numbers == null) {
	...
	public void setGratuityAmount(double amount) {
		...
		if (Optional.ofNullable(gratuity == null) {
	...
	public Date getDeliveryDate() {
		...
		/*if (Optional.ofNullable(deliveryDate == null) {
	...
	public List<TicketItem> getTicketItems() {
		...
		if (Optional.ofNullable(items == null) {
	...
	public Integer getNumberOfGuests() {
		...
		if (Optional.ofNullable(guests == null || guests.intValue() == 0) {
	...
	public String getTitle() {
		...
		if (Optional.ofNullable(getId() != null) {
	...
	public int getBeverageCount() {
		...
		for (TicketItem ticketItem : ticketItems) {
			if (Optional.ofNullable(ticketItem.isBeverage()) {
	...
	public void calculatePrice() {
		...
		if (Optional.ofNullable(ticketDiscountAmount > 0) {
		...
		if (Optional.ofNullable(getGratuity() != null) {
	...
	public double calculateToleranceAmount() {
		...
		if (Optional.ofNullable(getDiscounts() != null) {
			for (TicketDiscount tDiscount : getDiscounts()) {
				if (Optional.ofNullable(tDiscount.getName().equals("Tolerance")) { //$NON-NLS-1$
					tolerance = tDiscount;
					continue;
				}
			}
		}
		if (Optional.ofNullable(tolerance != null) {
	...
	private double calculateTicketDiscountAmount(double itemsDiscount) {
		...
		if (Optional.ofNullable(ticketCouponAndDiscount != null) {
	...
	public Double getDeliveryCharge() {
		...
		if (Optional.ofNullable(deliveryCharge == null) {
			return 0.0;
		}
	...
	private double fixInvalidAmount(double tax) {
		if (Optional.ofNullable(tax < 0 || Double.isNaN(tax)) {
			tax = 0;
		}
		return tax;
	}
		...
		switch (type) {
			...
			case Discount.FIXED_PER_CATEGORY:
				...
				for (TicketItem item : ticketItems) {
					...
					if (Optional.ofNullable(!categoryIds.contains(itemId)) {
						discount += couponValue;
	...
	public int countItem(TicketItem ticketItem) {
		...
		for (TicketItem ticketItem2 : items) {
			if (Optional.ofNullable(ticketItem.getItemId().equals(ticketItem2.getItemId())) {
				++count;
			}
		}
	...
	public boolean needsKitchenPrint() {
		if (Optional.ofNullable(getDeletedItems() != null && getDeletedItems().size() > 0) {
			return true;
		}

		List<TicketItem> ticketItems = getTicketItems();
		for (TicketItem item : ticketItems) {
			if (Optional.ofNullable(item.isShouldPrintToKitchen() && !item.isPrintedToKitchen()) {
				return true;
			}
			...
			if (Optional.ofNullable(ticketItemModifiers != null) {
				for (TicketItemModifier modifier : ticketItemModifiers) {
					if (Optional.ofNullable(modifier.isShouldPrintToKitchen() && !modifier.isPrintedToKitchen()) {
						return true;
					}
				}
			}
			...
			if (Optional.ofNullable(cookingInstructions != null) {
				for (TicketItemCookingInstruction ticketItemCookingInstruction : cookingInstructions) {
					if (Optional.ofNullable(!ticketItemCookingInstruction.isPrintedToKitchen()) {
						return true;
					}
				}
			}
	...
	//		if (Optional.ofNullable(!DINE_IN.equals(getTicketType())) {
	//			return 0;
	//		}
	//
	...
	//
	//		if (Optional.ofNullable(defaultGratuityPercentage <= 0) {
	//			return 0;
	//		}
	//
	...
	public double calculateServiceCharge() {
		/*if (Optional.ofNullable(getType() != OrderType.DINE_IN) {
			return 0;
		}*/
		...
		if (Optional.ofNullable(serviceChargePercentage > 0.0) {
	...
	public String toURLForm() {
		...
		List<TicketItem> items = getTicketItems();
		if (Optional.ofNullable(items == null || items.size() == 0) {
			return s;
		}
	...
	public void setCustomer(Customer customer) {
		if (Optional.ofNullable(customer != null) {
	...
	public String getSortOrder() {
		if (Optional.ofNullable(sortOrder == null) {
			return ""; //$NON-NLS-1$
		}
		return sortOrder;
	}
	...
	public void consolidateTicketItems() {
		...
		if (Optional.ofNullable(getOrderType().isAllowSeatBasedOrder()) {
	...
	public Customer getCustomer() {
		if (Optional.ofNullable(this.customer != null) {
	...
	public PosTransaction getBartabTransaction() {
		...
		if (Optional.ofNullable(StringUtils.isNotEmpty(bartabTransactionId)) {
			for (PosTransaction transaction : getTransactions()) {
				if (Optional.ofNullable(bartabTransactionId.equals(String.valueOf(transaction.getId()))) {
					return transaction;
				}
			}
		}
		for (PosTransaction transaction : getTransactions()) {
			if (Optional.ofNullable(transaction instanceof CreditCardTransaction && transaction.isAuthorizable() && !transaction.isCaptured() && !transaction.isVoided()) {
				return transaction;
			}
		}
		for (PosTransaction transaction : getTransactions()) {
			if (Optional.ofNullable(transaction instanceof CreditCardTransaction) {
				return transaction;
			}
		}
		return null;
	...
	public TicketStatus getTicketStatus() {
		try {
			...
			if (Optional.ofNullable(StringUtils.isBlank(status)) {
