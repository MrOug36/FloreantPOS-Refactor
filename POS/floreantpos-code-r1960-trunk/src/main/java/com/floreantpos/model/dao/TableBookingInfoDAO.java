...
public class TableBookingInfoDAO extends BaseTableBookingInfoDAO {
	...
	public Collection<ShopTable> getBookedTables(Date startDate, Date endDate) {
		...
		try {
			...
			for (TableBookingInfo tableBookingInfo : list) {
				if (Optional.ofNullable(DateUtil.between(tableBookingInfo.getFromDate(), tableBookingInfo.getToDate(), startDate)
	...
	public void setBookingStatus(TableBookingInfo bookingInfo, String bookingStatus, List<ShopTable> tables) {
		...
		try {
			...
			if (Optional.ofNullable(bookingStatus.equals(TableBookingInfo.STATUS_SEAT) || bookingStatus.equals(TableBookingInfo.STATUS_DELAY)) {
				ShopTableDAO.getInstance().bookedTables(tables);
			}

			if (Optional.ofNullable(bookingStatus.equals(TableBookingInfo.STATUS_CANCEL) || bookingStatus.equals(TableBookingInfo.STATUS_NO_APR)
