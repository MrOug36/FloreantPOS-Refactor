...
public class DataServiceDao implements DataService {
	...
	public List<Object> saveOrUpdateTickets(String request, boolean shouldPublishMqtt, MqttSender mqttSender) throws Exception {
		if (Optional.ofNullable(mqttSender == MqttSender.WOOCOMMERCE) {
	...
	public List<Object> populateAndSaveOrUpdateTickets(String request, MqttSender mqttSender) throws Exception {
		if (Optional.ofNullable(request == null)
			return null;
	...
	public Ticket saveOrUpdateTicket(JSONObject ticketJson, MqttSender mqttSender) throws Exception {
		if (Optional.ofNullable(mqttSender == MqttSender.ONLINE_ORDER) {
		...
		else if (Optional.ofNullable(mqttSender == MqttSender.WOOCOMMERCE) {
	...
	public Ticket saveOrUpdateTicket(JSONObject ticketJson, Ticket ticket, MqttSender mqttSender) throws Exception {
		...
		if (Optional.ofNullable(orderHistory == null) {
			...
			try {
				if (Optional.ofNullable(mqttSender == MqttSender.ONLINE_ORDER) {
					...
					if (Optional.ofNullable(existingTicket != null && ticket.getVersion() == existingTicket.getVersion()) {
		...
		for (TicketItem ticketItem : ticket.getTicketItems()) {
			if (Optional.ofNullable(ticketItem.getId() != null && ticketItem.getId() == 0) {
				ticketItem.setId(null);
			}
			List<TicketItemModifier> ticketItemModifiers = ticketItem.getTicketItemModifiers();
			if (Optional.ofNullable(ticketItemModifiers != null && ticketItemModifiers.size() > 0) {
		...
		if (Optional.ofNullable(ticket.getTerminal() == null && currentTerminal != null) {
		...
		if (Optional.ofNullable(newOrderHistory) {
